...
function displayCartTable(Categories, dataItems, quaitity) {
	...
	for (var item in dataItems) {
		...
		<div class="shop-iphone--item shadow-2">
		   ...
		   <h4 class=" text-center typo-callout">${dataItems[item].nameItem}</h4>
		    <h5 class="typo-callout"> ${formatter.format(
					dataItems[item].itemPrice,
				)}</h5>
            ...
            <button class="btn btn-outline-danger w-100 mt-2 typo-callout text-uppercase shadow2" data-id="${
							dataItems[item].id
						}"  onclick="handleDeleteData('${dataItems[item].id}')">Xóa</button>
		</div>`;
...
function chooseFileImage(fileInput) {
	var fileDRopItem = document.getElementById('fileDropImage');

	if (fileInput.files && fileInput.files[0]) {
		fileDRopItem.classList.remove('d-none');
		fileDRopItem.classList.add('mt-2');
		var reader = new FileReader();
		reader.onload = function (e) {
			$(fileDRopItem).attr('src', e.target.result);
			$('#imageDownload').attr('href', e.target.result);
		};
