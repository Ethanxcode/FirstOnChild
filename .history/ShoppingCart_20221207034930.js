...
promise.then(function () {
	var shoppingCart = (function () {
		...
		function handleUpdateCartData(data, callback, event) {
		...
		obj.totalCount = () => {
			...
			if (user == null) {
				...
				for (var item in user.cart) {
					totalCount += user.cart[item].count;
				}
			}
			return totalCount;
		};
		...
		obj.totalCart = () => {
			...
			if (user == null) {
				...
				for (var item in user.cart) {
					totalCart += user.cart[item].price * user.cart[item].count;
				}
			}
			copyTotal = totalCart;
