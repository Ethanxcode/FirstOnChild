...
function toggleLight() {
	...
	if (myLight) {
		myBody.classList.add('LightBackground');
		myLight = false;
	} else {
		myBody.classList.remove('LightBackground');
		myLight = true;
	}
}
...
var justDroppedItems = [
	...
	// 	itemDescription: 'Release: 12/29/2022',
	// },
	// {
	// 	id: 'NikeDunkLow002',
	...
	// 	itemDescription: 'Release: 06/23/2018',
	// },
	// {
	// 	id: 'AirJordan4004',
	// 	nameItem: 'Pink Riot',
...
async function loadDataFromMyDb() {
	...
	let data = await fetch(apiUrl)
		...
		.then((data) => {
			...
			data.forEach((item) => {
				// console.log(item);
				if (item.itemCategory == 'New') {
					latestNewsItems.push(item);
				}
				if (item.sold >= 7) {
				...
				if (
					...
					justDroppedItems.push(item);
				}
			});
			return latestNewsItems, bestSellerItems, justDroppedItems;
		})
		.then((data) => {
			...
			console.log(justDroppedItems);
			displayCartTable('justDropped', justDroppedItems, 3);
			displayCartTable('latestNews', latestNewsItems, 3);
			displayCartTable('bestSeller', bestSellerItems, 3);
		});
}
...
function displayCartTable(Categories, dataItems, quaitity) {
	...
	for (var item in dataItems) {
		...
		<div class="shop-iphone--item shadow-2">
		    <div class="shop-iphone--item---iphone">
		        <img src="${dataItems[item].itemImage}" alt="">
		    </div>
		    <h4 class=" text-center">${dataItems[item].nameItem}</h4>
		    <h5 > ${formatter.format(dataItems[item].itemPrice)}</h5>
		     <h6 > ${dataItems[item].itemRelease}</h6>
			 <p> ${dataItems[item].itemDescription}</p>
		    <div class="row">
		    <div class="col-6 "><a href="#" 
			data-id="${dataItems[item].id}" 
			data-name="${dataItems[item].nameItem}" 
			data-price="${dataItems[item].itemPrice}" 
			data-image="${dataItems[item].itemImage}" 
			data-status="${dataItems[item].status}
		data-status="${dataItems[item].status}
		data-brands="${dataItems[item].brands}" data-Category="${
			dataItems[item].itemCategory
		}
			itemCategory,
			brands, class="add-to-cart btn btnGreen w-100 typo-callout text-uppercase">Add to
							cart</a></div>
		            <div class="col-6"><a href="#" class="typo-callout text-uppercase btn btnPurple w-100">More</a></div>
		            </div>
			</div>
		</div>`;
