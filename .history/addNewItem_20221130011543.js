...
function displayCartTable(datas) {
	var list = document.querySelector('#justDropped');
	var htmls = datas.map(function (dataItems) {
		`<div class="col-xl-4 col-md-6 col-12">
		<div class="shop-iphone--item">
		    <div class="shop-iphone--item---iphone">
		        <img src="${dataItems.itemImage}" alt="">
		    </div>
		    <h5 class=" text-center">${dataItems.nameItem}</h5>
		    <h6> ${formatter.format(dataItems.itemPrice)}</h6>
		     <p> ${dataItems.itemDescription}</p>
		    <div class="row">
		    <div class="col-6 "><a href="#" 
			data-id="${
				dataItems.id
			}" data-name="${dataItems.nameItem}" data-price="${dataItems.itemPrice}" 
			 data-image="${
					dataItems.itemImage
            ...
            <button onlick="handleDeleteData(this)"></button>
		</div>`;
	});
